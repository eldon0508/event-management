...
<html>
    <head>
        <meta charset="UTF-8">
        <title>LYGL | Reactivate Account</title>
    </head>
    <body>
        <header><?php include './header.php';?></header>
        <div style="padding-right: 28%;padding-left: 28%;">
            <h3 style="text-align: center;">Account Reactivate</h3>
            ...
            if(!empty($err)){
                ...
                while($row = $run->fetch_object()){
                    ...
                    if(checkForget($dID,$id,$dEmail,$email,$dQ1,$q1,$dQ2,$q2,$dA1,$a1,$dA2,$a2)){
                        //matched user
                        $code = "update member set u_status = 'A' where username = '$id'";
                        if($run = $db->query($code)){     //show success message
                            echo "<script>alert('Your account has been reactivated!');window.location.href='login.php';</script>";
                        }else{
                            //show failed message
                            echo "<script>alert('Reactivation failed! Try Again!');window.location.href='reactiveAccount.php';</script>";
                        }
                    }else{     //not matched user
        ...
        ?>
                ...
                <div class="btm" style="text-align: center;">
                    <button type="submit" name="activate"
                            class="waves-effect waves-light btn-large" style="width: 200px;background-color: #00bcd4;">Activate</button>
                </div>
